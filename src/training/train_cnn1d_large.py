...
from src.models.cnn1d_large import CNN1DLarge
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a large 1D CNN classifier.")
...
def main():
    ...
    model = CNN1DLarge(in_channels=data["train"]["X"].shape[2])
    ...
    run_dir = make_run_dir(
        args.run_root, args.dataset, "classification", "cnn1d_large", args.model_seed
    )
    ...
    print(f"Saved large CNN1D run to {run_dir}")
