...
def main():
    ...
    model = IsolationForest(
        ...
        contamination=_auto_or_float(args.contamination),
        max_samples=_auto_or_float(args.max_samples),
...
def _run_dir(run_root, dataset, seed):
    return Path(run_root) / dataset / "tsad" / "isolation_forest" / f"seed_{seed}"


def _auto_or_float(value):
    if value == "auto":
        return value
    return float(value)
